...
error_t Keypad_Init(keypad_t * pKeypad)
{
    ...
    {
        GPIO_SetPinPullup(pKeypad->Keypad_RowArr[i].port, pKeypad->Keypad_RowArr[i].pin);
    }
    /*--------Set Columns Pin as Output Pin---------*/
    for (i = 0; i < Keypad_numberOfCols; i++)
    {
        GPIO_SetPinDirection(pKeypad->Keypad_COLArr[i].port, pKeypad->Keypad_COLArr[i].pin, kOutput);
    }
    /*--------Set Columns Pin as High---------------*/
    for (i = 0; i < Keypad_numberOfCols; i++)
    {
        GPIO_SetPinValue(pKeypad->Keypad_COLArr[i].port, pKeypad->Keypad_COLArr[i].pin, kHigh);
    }
}
uint8_t Keypad_GetPressedButton(void)
{
    ...
    {
        GPIO_SetPinValue(pKeypad->Keypad_COLArr[i].port, pKeypad->Keypad_COLArr[i].pin, klow);
        for (rowCount = 0; rowCount < Keypad_numberOfRows; rowCount++)
        {
            GPIO_GetPinValue(Keypad_RowsPORT, Keypad_RowArr[rowCount], &buttonValue);
        ...
        GPIO_SetPinValue(Keypad_ColsPORT, Keypad_COLArr[colCount], kHigh);
    }
    return retValueButton;
}
